...
public class SysRole extends BaseSysRole<SysRole> {
	...
	{
	return SysRole.dao.find("select * from sys_role");		
	}
	 ...
	 * @return
	 */
	public Page<SysRole> getRolePage(Integer curr , Integer pagesize,String Statustype,String Searcht){
		if(Searcht==""){
			if(Statustype=="")
		{  	return SysRole.dao.paginate(curr, pagesize, "select * ", " from sys_role");};
	  }
	  else{
			return SysRole.dao.paginate(curr, pagesize, "select *", " from sys_role where CONCAT(name,status,description) like concat('%', '"+Searcht+"','%') ");
	  }
  
	return SysRole.dao.paginate(curr, pagesize, "select * ", " from sys_role where status = '"+Statustype+"'");
	}
	/***
	 * 删除角色下所有权限
	 * @param roleid
	 ...
	 * @return
	 */
	public String findIdByuserRoleId(Integer userRoleId) {
		SysRole sysRole = SysRole.dao.findFirst("select * from sys_role where orderId = ?", userRoleId);
